# Skip-thought vectors implementation to create skip-thought model, encodings, etc.
# Implementation taken from the given github link : https://github.com/ryankiros/skip-thoughts
...
# Test function to check if skip-thoughts have been imported in data_loader.py
def test():
	print("skipthoughts have been imported successfully!")
...
# Used indirectly by data_loader, called by load_model() function above.
def load_tables():
	"""
	Load the tables
	"""
...
# Used to create encodings for the given text. Used in data_loader.py
...
# Used to preprocess the text received by the encode() function above.
def preprocess(text):
	"""
...
def init_params_bi(options):
	...
	# encoder: GRU
	params = get_layer(options['encoder'])[0](options, params, prefix='encoder', nin=options['dim_word'], dim=options['dim'])
	params = get_layer(options['encoder'])[0](options, params, prefix='encoder_r', nin=options['dim_word'], dim=options['dim'])
	return params
...
def gru_layer(tparams, state_below, options, prefix='gru', mask=None, **kwargs):
	...
	rval, updates = theano.scan(_step, sequences=seqs, outputs_info = [tensor.alloc(0., n_samples, dim)],
								...
								name=_p(prefix, '_layers'), n_steps=nsteps, profile=profile, strict=True)
